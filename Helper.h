...
DSHDEF int ds_io_write_file(const char *path, const char *buffer,
                            const char *mode);
    ...
    if ((da)->count >= (da)->capacity) {                                       \
      ...
      if (new_capacity == 0) {                                                 \
        new_capacity = DS_DA_INIT_CAPACITY;                                    \
                                                                               ...
                                                                               \
      (da)->items =                                                            \
          DS_REALLOC(NULL, (da)->items, (da)->capacity * sizeof(*(da)->items), \
...
DSHDEF int ds_string_builder_append(ds_string_builder *sb, const char *format,
  ...
  if (ds_dynamic_array_append_many(&sb->items, (void **)buffer, needed) != 0) {
    return_defer(1);
  }
...
DSHDEF int ds_dynamic_array_append(ds_dynamic_array *da, const void *item) {
  ...
  DS_MEMCPY((char *)da->items + da->count * da->item_size, item, da->item_size);
  da->count++;
...
DSHDEF void ds_argparse_parser_init_allocator(ds_argparse_parser *parser,
  ...
  ds_argparse_add_argument(
      parser, (ds_argparse_options){.short_name = 'v',
                                    .long_name = "version",
...
static int argparse_validate_parser(ds_argparse_parser *parser) {
  ...
  for (size_t i = 0; i < parser->arguments.count; i++) {
    ...
    if (options.type == ARGUMENT_TYPE_FLAG && options.required == 1) {
      DS_LOG_ERROR("flag argument cannot be required: %s", options.long_name);
      result = 1;
    }
...
static int argparse_post_validate_parser(ds_argparse_parser *parser) {
  ...
  for (size_t i = 0; i < parser->arguments.count; i++) {
    ...
    if (options.type == ARGUMENT_TYPE_VALUE && options.required == 1) {
      if (item->value == NULL) {
        DS_LOG_ERROR("missing required argument: --%s", options.long_name);
        result = 1;
      }
    }

    if (options.type == ARGUMENT_TYPE_VALUE_ARRAY && options.required == 1) {
    ...
    if (name[0] == '-') {
      ...
      case ARGUMENT_TYPE_VALUE_ARRAY: {
        ...
        if (ds_dynamic_array_append(&arg->values, &argv[++i]) != 0) {
          DS_LOG_ERROR("failed to append value to argument: %s", name);
          return_defer(1);
        }
        break;
      }
...
DSHDEF int ds_io_write_file(const char *filename, const char *buffer,
                            const char *mode) {
  int result = 0;
  FILE *file = NULL;
